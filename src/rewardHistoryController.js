...
class RewardHistoryController {
  ...
  async getAllRewardHistory() {
    try {
      const sql = "SELECT * FROM RewardHistory";
  ...
  async addRewardHistory(uid, timestamp, rid, status) {
    try {
      const sql = "INSERT INTO RewardHistory (uid, timestamp, rid, status) VALUES (?, ?, ?, ?)";
  ...
  async updateRewardHistory(rewardHistoryId, uid, timestamp, rid, status) {
    try {
      const sql = "UPDATE RewardHistory SET uid = ?, timestamp = ?, rid = ?, status = ? WHERE rhid = ?";
      await this.query(sql, [uid, timestamp, rid, status, rewardHistoryId]);
